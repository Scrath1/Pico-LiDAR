...
static inline bool operator==(const T& lhs, const T& rhs) {
    return (lhs.value == rhs.value) && (lhs.lastChanged_ms == rhs.lastChanged_ms);
}
...
static inline bool areEqualAbs(float a, float b, float epsilon) { return (std::fabs(a - b) <= epsilon); }

static inline bool operator==(const setting<float>& lhs, const setting<float>& rhs) {
    return (areEqualAbs(lhs.value, rhs.value, FLOAT_EQUALITY_THRESHOLD)) && (lhs.lastChanged_ms == rhs.lastChanged_ms);
}
